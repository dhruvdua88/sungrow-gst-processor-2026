...
// For each AP / AR party, decompose what HITS its account: the contra (non-party)
// lines in every voucher the party appears in, bucketed by category. When a
// voucher has multiple parties, the contra is split pro-rata by party amount.
// Repeatable rule derived from analysing all AP entries (covers ~99.9%):
...
export const MATRIX_CATS = ["bank", "expense", "inventory", "tds", "gst", "other"];
function contraCat(t) {
  const g = t.gl_code, lt = t.lineType;
...
export function runTxnMatrix(txns) {
  const byDoc = new Map();
  for (const t of txns) { const k = t.voucher_no || "row-" + t.rowIndex; const a = byDoc.get(k); if (a) a.push(t); else byDoc.set(k, [t]); }
  const acc = new Map(); // party -> row
  const blank = (party, kind) => ({ party, kind, total: 0, bank: 0, expense: 0, inventory: 0, tds: 0, gst: 0, other: 0, vouchers: 0 });
  ...
  for (const [doc, lines] of byDoc) {
    ...
    const kind = apLines.length ? "AP" : "AR";

    // contra = everything that is not a party line of this kind
    const partySet = new Set(partyLines);
    const cat = { bank: 0, expense: 0, inventory: 0, tds: 0, gst: 0, other: 0 };
    for (const l of lines) {
      if (partySet.has(l)) continue;
      cat[contraCat(l)] += Math.abs(l.debit - l.credit);
    }

    // per-party amount (own movement) and pro-rata weight
    ...
    partyLines.forEach((l, i) => {
      ...
      if (!row) { row = blank(l.vendor, kind); acc.set(l.vendor, row); }
  ...
  const totals = (list) => {
    const t = blank("ALL", "");
    for (const p of list) { t.total += p.total; for (const c of MATRIX_CATS) t[c] += p[c]; t.vouchers += p.vouchers; }
    return t;
  };
...
export function analyzeDaybook(txns) {
  return { mis: runMis(txns), sales: runSales(txns), matrix: runTxnMatrix(txns) };
}
